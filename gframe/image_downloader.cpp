...
void ImageDownloader::DownloadPic() {
  ...
  auto SetPayloadAndUrl = [&payload, &curl](epro::stringview url,
                                            FILE *stream) {
    ...
    for (auto &src : pic_urls) {
      ...
      if (fp == nullptr) {
        if (gGameConfig->logDownloadErrors) {
          ygo::ErrorLog("Failed opening {} for write.",
                        Utils::ToUTF8IfNeeded(name));
          ygo::ErrorLog("Error: {}.", strerror(errno));
        }
        continue;
      }
      SetPayloadAndUrl(epro::format(src.url, code), fp);
      ygo::ErrorLog("{}", epro::format(src.url, code));
      ...
      if (gGameConfig->logDownloadErrors) {
        ...
        ygo::ErrorLog("Curl error: ({}) {} ({})",
                      static_cast<std::underlying_type_t<CURLcode>>(res),
...
ImageDownloader::downloadStatus
ImageDownloader::GetDownloadStatus(uint32_t code, imgType type) {
  if (type == imgType::THUMB)
    type = imgType::ART;
...
epro::path_stringview ImageDownloader::GetDownloadPath(uint32_t code,
                                                       imgType type) {
  if (type == imgType::THUMB)
    type = imgType::ART;
  std::lock_guard<epro::mutex> lk(pic_download);
